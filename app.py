...
def generate_expert_engine(input_text):
    try:
        ...
        return model.generate_content(input_text).text, "🟢 Prapali-Engine"
    except Exception:
        if OPENROUTER_KEY:
            try:
                ...
                return response.json()['choices'][0]['message']['content'], "🟡 Prapali-Backup"
            except:
                raise Exception("ท่อสัญญาณหนาแน่น กรุณาลองใหม่ใน 30 วินาที")
        else:
            raise Exception("ระบบหลักหนาแน่น และไม่พบกุญแจระบบสำรอง")

# ==============================================================================
# 2. UI และการกู้คืนลิขสิทธิ์/ลิงก์ติดต่อ
# ==============================================================================
...
.footer-link { color: #c5a85c !important; text-decoration: none !important; font-size: 14px; margin: 0 10px; }
.provider-tag { font-size: 10px; color: #444; text-align: right; margin-top: 5px; }
</style>
...
if user_input := st.chat_input("พิมพ์ข้อความเพื่อสั่งงานวิศวกร prapali..."):
    ...
    with st.chat_message("assistant"):
        with st.spinner("กระผม prapali กำลังประมวลผลข้อมูล..."):
            try:
                ...
                st.error(f"❌ รายงานข้อผิดพลาด: {e}")

# --- ส่วนลิขสิทธิ์และลิงก์ติดต่อที่กู้คืนมา ---
st.markdown("""
<div class="support-card">
    <div style="color: #c5a85c; font-size: 16px; font-weight: bold; margin-bottom: 12px;">☸️ สนับสนุนระบบปัญญาประดิษฐ์พระบาลี</div>
    
    <details style="background-color: #161616; border: 1px solid #2d2d2d; border-left: 4px solid #c5a85c; padding: 12px; border-radius: 6px; text-align: left; cursor: pointer; margin-bottom: 15px;">
        <summary style="color: #c5a85c; font-size: 14px; font-weight: bold;">🏦 รายละเอียดบัญชีสนับสนุน</summary>
        <div style="margin-top: 10px; border-top: 1px solid #2d2d2d; padding-top: 10px;">
            <div style="color: #ffffff; font-size: 16px; font-weight: 800;">777-438496-0</div>
    ...
    <div style="margin-top: 15px; border-top: 1px solid #2d2d2d; padding-top: 15px;">
        <a class="footer-link" href="tel:0644518043">📞 064-4518043</a>
        <a class="footer-link" href="https://www.facebook.com/prapali.ai" target="_blank">🔵 Facebook</a>
        <a class="footer-link" href="mailto:contact@prapali.ai">📧 Email</a>
    </div>
    
    <div style="color: #555; font-size: 11px; margin-top: 15px;">
        © 2026 AI.prapali - วิศวกรรมปัญญาประดิษฐ์เพื่อพระธรรมวินัย <br>
        สงวนลิขสิทธิ์โดย วิศวกร prapali และทีมพัฒนา
    </div>
</div>
""", unsafe_allow_html=True)
